...
Auto-Resubmit 目前仍然是一个基于规则的初步版本。

它已经能够完成支持会议家族之间的主要模板迁移流程，但并不意味着已经覆盖所有科研论文里可能出现的 LaTeX 写法。部分论文会带有比较复杂的自定义前页结构、自定义环境、特殊首页逻辑、非常规附录入口，或者宏包之间的特殊交互；这些情况当前规则不一定都已经考虑完整。

通常来说，在下面这些条件下，转换更容易成功：

- 源输入是标准的 LaTeX 项目 zip
- 源项目只有一个清晰的主稿入口
- 图片、参考文献和本地 `.sty` 文件都已经包含在项目里
- 论文整体基本遵循常见会议模板结构
- 存在自定义宏，但没有大幅重写整篇文档结构

下面这些情况则更可能还需要手工修补：

- 论文使用了高度定制的首页或 frontmatter 逻辑
- abstract、appendix、bibliography 通过非常规包装方式插入
- 源项目依赖自定义编译脚本或外部资源
- 源稿定义了当前规则还无法正确识别的特殊命令或环境

但大的转换骨架已经有了，所以很多时候问题并不是“完全不能转”，而是“还差一些规则上的细节补齐”。

如果你遇到这类边界情况，非常欢迎你 fork 这个仓库，再结合 Codex 或 Claude Code 之类的工具，针对自己的模板或论文把规则补齐。对于这类问题，通常做少量规则修复就能解决。
...
作者也在持续收集更多真实会议模板和源论文样例，不断完善现有规则，让转换器随着时间变得更稳、更通用。
